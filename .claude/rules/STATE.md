...
| Full suite passing (baseline) | **1004** (+9 from PR #245 pending baseline auto-update) |
| Last verified | 2026-06-10 on `main` (post PR #245) |
...
- **2026-06-10** — PR #244 ([OSS-481](https://linear.app/kunai/issue/OSS-481), Done) merged to `main` (`29c65ee`). createOptimizer now speaks SWC's raw-string NAPI surface — plain-string options in, SWC-shaped records out, brands established internally at the boundary — removing the brand/cast seam from qwik-bundler's `createTsOptimizer` adapter (OSS-450 territory). +2 tests (full baseline 1002→1004); convergence unchanged.
