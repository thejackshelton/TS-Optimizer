...
Last updated: 2026-07-10 late pm (OSS-529 **production-path** click crash FIXED — clean-cache re-sweep proved the earlier fix was incomplete; OSS-534 filed for the remaining partition + select crash). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`). Full dev+prod SSR **route** render parity TS≡SWC is done (all 29 component routes). The active layer is the **interaction sweep** — a side-by-side TS-vs-SWC Playwright pass to verify browser-interaction parity. **[OSS-529](https://linear.app/kunai/issue/OSS-529) `fn.apply is not a function` click crash — now FIXED on the PRODUCTION path (PR #348, In Review, stacked on #347).** Critical finding: a clean-cache re-sweep (build + rsync dist into the qds pnpm store copy + `rm -rf docs/node_modules/.vite` + restart) showed the OSS-529/530 partition fix landed only in the **raw-JSX** path (`jsx-elements-core.ts`) that convergence snapshots exercise — but the real bundler feeds pre-transformed **`_jsxDEV(...)`** JSX through `buildJsxSortedCall` (`jsx-call-transform.ts`), which was untouched, so **7 routes still crashed** (checkbox/collapsible/otp/pagination/radio-group/tabs/textbox). Root cause (runtime-traced via patched `runEventHandlerQRL`): a `$`-suffixed event handler with a reactive ternary value (`onPointerEnter$: hover ? [hover.handleIn$, props.onPointerEnter$] : …`) was hoisted to `_fnSignal`; the runtime dispatched the resulting `WrappedSignal` as the handler. Fix (new commit on #348): `wrapReactivePropValues` skips `$`-keys (`isHandlerPropKey`) + `buildJsxSortedCall` routes `$`-key component props to the var bag raw. Folded in a parity fix found en route: `bind:X={obj.field}` on a component now emits `_wrapProp(obj,"field")` (`jsx-props.ts`). **Browser-verified crash-free on all 7 routes + popover + select-embedding pages against PRISTINE qwik-core.** +4 focused regression tests. **[OSS-534](https://linear.app/kunai/issue/OSS-534) filed (In Progress)** for the two remaining items, both one story: (1) the production `_jsxDEV` path still emits a `null` const bag where SWC partitions (`buildJsxSortedCall` — needs the OSS-529 before/after-spread partition ported); (2) `/components/select/` still crashes (`insertBefore` on a Switch toggle; TS-specific, SWC clean) — likely the same partition gap. **`modal`'s `[generateJsx]` error is NOT ours** — reproduces on SWC too (docs playground tooling). Still open from earlier: [OSS-531](https://linear.app/kunai/issue/OSS-531) (`sync$` TS-annotation leak, verified, Backlog), [OSS-532](https://linear.app/kunai/issue/OSS-532) (Textbox `readOnly` no-effect, Backlog). **KEY unlock:** offline lowered-input harness (esbuild `jsxDev` → `transformModule`) reproduces the `_jsxDEV`-path codegen divergence without a browser — this is how the production gap was isolated; runtime tracing still needs the patch-core + clear-`.vite` recipe. Convergence 203/9 unchanged; full-suite gate green, +11 tests land on merge of #348. OSS-456 parity + OSS-450 integration open in parallel.
...
| Full suite passing (baseline) | **1158** (+11 land on merge of PRs #347 + #348) |
| Last verified | 2026-07-10 late pm on `fix/oss-529-jsxsplit-partition` @ `2d4dbe4` (regression gate green vs baseline; +11 new tests; typecheck clean) |
...
| `fix/raw-props-default-sibling-ref` | PR #347 → `main` | ✅ | gate green | [OSS-533](https://linear.app/kunai/issue/OSS-533) (In Review) — raw-props default consolidation strands a sibling-binding reference (`prefetchProp is not defined` SSR crash, regression from OSS-528). Fix: `defaultExprIsNonConst` gate in `rewrite/raw-props.ts`. +2 tests. |
| `fix/oss-529-jsxsplit-partition` | `2d4dbe4` (PR #348, stacked on #347) | ✅ | gate green | [OSS-529](https://linear.app/kunai/issue/OSS-529) + [OSS-530](https://linear.app/kunai/issue/OSS-530) (In Review) — `fn.apply` click crash. Raw-JSX partition (`jsx-elements-core.ts`) + `!signal.value` hoist (`signal-analysis.ts`), **plus the production-path fix** (`jsx-call-transform.ts` handler-hoist guard + `jsx-props.ts` bind `_wrapProp`) that clears the 7 routes the raw-JSX-only fix missed. Browser-verified vs pristine core. +11 tests. Residual → [OSS-534](https://linear.app/kunai/issue/OSS-534). |
...
**Immediate — [OSS-534](https://linear.app/kunai/issue/OSS-534) (In Progress):** port the OSS-529 const-bag partition from the raw-JSX path (`jsx-elements-core.ts`) to the production `_jsxDEV` path (`buildJsxSortedCall` in `jsx-call-transform.ts`), which still emits `null` const bags on every spread-carrying component. Must handle the **before/after-spread** distinction (`tabs-root` → bare `_getConstProps(rest)` const bag; `popover-root` → const props in `{…}`). Target: byte-parity on select-root/popover-root/tabs-root + `/components/select/` no longer crashes (`insertBefore` on Switch toggle). Update the `jsxdev-reactive-const-bag.test.ts` "spread on a component" test (currently asserts the null-const-bag form). **Session scratchpad tooling** (`/private/tmp/claude-501/-Users-scottweaver-Projects-TS-Optimizer/889d39b0-.../scratchpad/`): `repro-jsxdev.mjs` (lowered `_jsxDEV` TS-vs-SWC per-file), `norm-diff-lowered.mjs` (normalized full-module lowered diff), `batch-scan.mjs` (corpus null-const-bag counts), `sweep-all.mjs` / `pinpoint.mjs` (browser interaction + crash pinpoint), `pw529b.mjs`, `repro529.mjs`. The offline lowered-input harness (esbuild `jsxDev` → `transformModule`, opts `{minify:'simplify',mode:'hmr',entryStrategy:'smart',isServer:false}`) reproduces the divergence without a browser. For runtime tracing: patch qwik-core `core.mjs` `runEventHandlerQRL` + `rm -rf docs/node_modules/.vite` + restart (`QDS_TS_OPTIMIZER=1 vite --mode ssr --port 5200`); a `.orig-bak` restore path is the cleanup.
