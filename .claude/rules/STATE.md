...
| Full suite passing (baseline) | **1158** (+11 on #348, +14 more on OSS-534) |
| Last verified | 2026-07-14 on `fix/oss-534-jsxdev-const-bag-partition` @ `de2ddd7` (regression gate green vs baseline; +14 new tests; typecheck clean) |
...
| `fix/oss-534-jsxdev-const-bag-partition` | `de2ddd7` (stacked on #348) | local | gate green | [OSS-534](https://linear.app/kunai/issue/OSS-534) — port the `_jsxSplit` var/const-bag partition to the production `_jsxDEV` path (`buildJsxSortedCall`). select-root/popover-root/tabs-root byte-parity; corpus 133→28 divergent files. +14 tests. Not pushed yet (stacked behind #347/#348). Select `insertBefore` crash re-scoped → [OSS-535](https://linear.app/kunai/issue/OSS-535). |
...
**[OSS-534](https://linear.app/kunai/issue/OSS-534) is DONE** (committed `de2ddd7`, not pushed — stacked behind #347/#348). When #347→#348 clear review/merge, push `fix/oss-534-jsxdev-const-bag-partition` and open its PR (stacked on #348, or rebased onto `main` once #348 merges). It touches `src/` → normal review.
