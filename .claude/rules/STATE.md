...
Last updated: 2026-07-10 pm (OSS-529 dominant click crash FIXED + OSS-533 SSR regression fixed; two PRs open, both browser-verified). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`). Full dev+prod SSR **route** render parity TS≡SWC is done (all 29 component routes). The active layer is the **interaction sweep** — a side-by-side TS-vs-SWC Playwright pass to verify browser-interaction parity. **[OSS-529](https://linear.app/kunai/issue/OSS-529) + [OSS-530](https://linear.app/kunai/issue/OSS-530) — the DOMINANT `fn.apply is not a function` click crash on 10 routes — FIXED (PR #348, In Review, stacked on #347).** Root cause runtime-traced (settled 3 conflicting agent opinions): patched qwik-core `invokeApply`/`runEventHandlerQRL` → the dispatched "handler" is a `_fnSignal` WrappedSignal because reactive props sat in the VAR bag, shifting the runtime's positional handler resolution. Needed BOTH fixes: (OSS-529) component single-spread `_jsxSplit` now partitions const props into the const bag instead of dumping all to var with a `null` const bag (`partitionableComponentSpread`, `jsx-elements-core.ts`); (OSS-530) `!signal.value` (a `UnaryExpression`) now hoists to `_fnSignal` (`signal-analysis.ts`). Together the var bag holds only event handlers, byte-matching SWC on popover-root. All 10 routes (popover/radio-group/select/tabs/checkbox/collapsible/textbox/modal/otp/pagination) render 200 + no crash on click. **[OSS-533](https://linear.app/kunai/issue/OSS-533) — SSR-blocking regression from today's OSS-528 (PR #342), found while building the OSS-529 repro — FIXED (PR #347, In Review).** Fresh-cache dev SSR hung on every page (`prefetchProp is not defined` in router `Link`): raw-props default consolidation stranded a sibling-binding reference; fix widened the unsafe-default gate to a faithful `is_const_expr` port (`collectPatternBindings`). **NEXT: continue the interaction sweep across the remaining widget routes** — still open: [OSS-531](https://linear.app/kunai/issue/OSS-531) (`sync$` TS-annotation leak, verified, Backlog), [OSS-532](https://linear.app/kunai/issue/OSS-532) (Textbox `readOnly` no-effect, TS-specific, not root-caused, Backlog). **KEY unlock:** the vite-cache blocker is defeated by patching qwik-core `core.mjs` on disk + `rm -rf node_modules/.vite` + restart, then instrumenting `invokeApply`/`runEventHandlerQRL` — this is how the WrappedSignal was pinned. Convergence 203/9 unchanged (both PRs); full-suite gate green, +7 tests land on merge. OSS-456 parity + OSS-450 integration open in parallel.
...
| Full suite passing (baseline) | **1158** (+7 land on merge of PRs #347 + #348) |
| Last verified | 2026-07-10 pm on `fix/oss-529-jsxsplit-partition` (regression gate green vs baseline; +7 new tests) |
...
| `fix/oss-529-jsxsplit-partition` | PR #348 (stacked on #347) | ✅ | gate green | [OSS-529](https://linear.app/kunai/issue/OSS-529) + [OSS-530](https://linear.app/kunai/issue/OSS-530) (In Review) — the DOMINANT `fn.apply` click crash on 10 routes. `_jsxSplit` component single-spread const-bag partition (`jsx-elements-core.ts`) + `!signal.value` UnaryExpression hoist (`signal-analysis.ts`). Root cause runtime-traced. All 10 routes browser-verified crash-free. +5 tests. |
...
**Immediate (active workstream OSS-499):** continue the **interaction-parity sweep** — run the TS-vs-SWC Playwright pass across the remaining widget routes to surface more browser-level interaction bugs. Reusable tooling (current session scratchpad + prior fleet scratchpad): `pw529b.mjs` (targeted click-crash probe), `pw-probe.mjs` (broad auto-interaction), `scan-divergence.mjs` (offline TS↔SWC codegen diff), `repro529.mjs` (per-file `_jsxSplit` diff). Known-open interaction bugs to fold in: [OSS-531](https://linear.app/kunai/issue/OSS-531) (`sync$` TS-annotation leak, verified), [OSS-532](https://linear.app/kunai/issue/OSS-532) (Textbox `readOnly` no-effect, not root-caused). When the runtime is opaque, defeat the vite cache by patching qwik-core `core.mjs` on disk + `rm -rf node_modules/.vite` + restart, then instrument the dispatch (the technique that pinned OSS-529). Merge PRs #347 → #348 first (both touch `src/`, normal review) — then the branches rebase/clean up post-merge.
