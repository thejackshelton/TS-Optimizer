...
| Full suite passing (baseline) | **1187** on `main`; **1192** on `fix/oss-536-inline-hoist-usehmr` (+5 OSS-536 regression tests, PR pending) |
| Last verified | 2026-07-15 — OSS-536 branch gate green (203 convergence + 1187 full baseline-passing all pass; +5 new) |
...
| `fix/oss-536-inline-hoist-usehmr` | `4891e38` | (PR pending) | 203/9, +5 | **OSS-536 fix** — inject `_useHmr` into inline/hoist (dev-SSR) `component$` bodies in hmr mode, matching the segment (client) path. Closes the server/client HMR-hook desync that caused the `Missing refElement` assert. Browser-verified crash-gone on all 5 ref-bound routes. |
